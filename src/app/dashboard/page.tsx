...
import { collection, query, orderBy, doc, limit, onSnapshot } from 'firebase/firestore';
  ...
  return (
    <Card className="bg-card border-border border-2 rounded-3xl overflow-hidden shadow-2xl flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto max-h-[500px] custom-scrollbar divide-y divide-border">
        ...
        }) : <div className="p-12 text-center space-y-3"><Database className="h-8 w-8 text-muted-foreground mx-auto opacity-20" /><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">Awaiting Log Updates</p></div>}
      </div>
    </Card>
  );
}
  ...
  return (
    <Card className="bg-card border-border border-2 rounded-2xl overflow-hidden shadow-xl flex flex-col h-full group hover:glow-primary transition-all">
      <div className="relative aspect-video bg-muted flex items-center justify-center overflow-hidden">
        {bird?.primaryImageUrl ? <Image src={bird.primaryImageUrl} alt={bird.name || 'Resident'} fill className="object-cover transition-transform duration-700 group-hover:scale-110" /> : <span className="text-4xl">🦆</span>}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="absolute bottom-3 left-4"><h3 className="text-xl font-headline font-black text-white uppercase tracking-tighter">{bird?.name || 'Friend'}</h3><p className="text-[10px] text-primary font-black uppercase tracking-widest">{bird?.breed}</p></div>
      </div>
      <CardContent className="p-6 space-y-4 flex-1 flex flex-col">
        <div className="space-y-1"><div className="flex justify-between items-center text-[8px] font-black uppercase tracking-widest text-muted-foreground"><span>Health Status</span><span>{Math.round(dailyStatusProgress)}%</span></div><Progress value={dailyStatusProgress} className="h-1.5" /></div>
        <div className="pt-2 mt-auto"><Button asChild variant="outline" className="w-full h-9 rounded-xl text-[9px] font-black uppercase tracking-widest border-border hover:bg-primary/10"><Link href={`/residents/${bird.id}`}>View Profile <ChevronRight className="ml-1 h-3 w-3" /></Link></Button></div>
      </CardContent>
    </Card>
  );
}
