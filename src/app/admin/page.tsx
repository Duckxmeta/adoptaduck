...
export default function AdminDashboard() {
  ...
  const handleAddEgg = (resident: Resident) => {
    ...
    // Update global counter document
      ...
      <main className="container mx-auto p-4 space-y-8">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card className="bg-card text-card-foreground border border-border shadow-2xl rounded-2xl overflow-hidden relative">
            ...
            <CardContent className="p-6 relative z-10">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Aggregated Eggs</p>
              <p className="text-4xl font-headline font-black mt-1 text-primary">
                {birds?.reduce((sum, b) => sum + (b.eggCounter || 0), 0) || 0}
              </p>
            </CardContent>
          </Card>
        </div>
          ...
          {birdsLoading ? (
            ...
            return (
              <Card key={bird.id} className="bg-card text-card-foreground border-border rounded-2xl overflow-hidden shadow-xl group transition-all hover:border-primary/30">
                ...
                <div className={cn(
                  "grid border-t border-border divide-x divide-border",
                  isHen ? "grid-cols-5" : "grid-cols-3"
                )}>
                  {isHen && (
                    <>
                      <Button 
